...
#define get_ctrl_if() ((control_if_t *)((unsigned char *)HYPERVISOR_shared_info + 2048))
#define TX_FULL(_c)   \
...
int
ctrl_if_send_message_noblock(ctrl_msg_t *msg, ctrl_msg_handler_t hnd,
				unsigned long id)
{
	control_if_t *ctrl_if = get_ctrl_if();
	unsigned long flags;
...
int
ctrl_if_send_message_block(ctrl_msg_t *msg, ctrl_msg_handler_t hnd,
			unsigned long id, long wait_state)
{
	int rc;
...
int
ctrl_if_send_message_and_get_response(ctrl_msg_t *msg, ctrl_msg_t *rmsg,
				long wait_state)
{
...
#ifdef notyet
int
ctrl_if_enqueue_space_callback(struct tq_struct *task)
{
	control_if_t *ctrl_if = get_ctrl_if();
	...
	/*
	 * We may race execution of the task queue, so return re-checked
	 * status. If the task is not executed despite the ring being
	 * non-full then we will certainly return 'not full'.
	 */
	x86_lfence();
...
int
ctrl_if_register_receiver(uint8_t type, ctrl_msg_handler_t hnd,
			unsigned int flags)
{
	...
	if ( inuse ) {
		...
		x86_atomic_clear_bit(
			(unsigned long *)&ctrl_if_rxmsg_blocking_context, type);
...
ctrl_if_resume(void)
{
	...
	if (xen_start_info.flags & SIF_INITDOMAIN) {
		/*
		 * The initial domain must create its own domain-controller
		 * link.  The controller is probably not running at this point,
		 * but will pick up its end of the event channel from
		 */
		evtchn_op_t op;
		...
		xen_start_info.domain_controller_evtchn =
		    op.u.bind_interdomain.port1;
		initdom_ctrlif_domcontroller_port =
	...
	printf("Domain controller: using event channel %d\n", ctrl_if_evtchn);

	event_set_handler(ctrl_if_evtchn, &ctrl_if_interrupt, NULL,
	    IPL_SCHED, "ctrlev");
...
void ctrl_if_init(void)
{
	...
	kthread_create_deferred(ctrl_if_kthread_create, NULL);
...
ctrl_if_kthread_create(void *arg)
{
	...
	if ((error = kthread_create(__ctrl_if_rxmsg_deferred, NULL,
	    &ctrl_if_proc, "ctrlif")) != 0)
	{
		printf("ctrlif: unable to create kernel thread: error %d\n",
		    error);
	}
}
